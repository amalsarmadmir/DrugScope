...
class ReportExporter:
    ...
    def to_json(self, output_path: Path) -> None:
        ...
        json_payload = {
            ...
            "age_demographics": {
                "average_age": round(self.results["age_demographics"]["average_age"], 2),
                "cohort_distribution": self.results["age_demographics"]["cohorts"].to_dict(orient="records")
            },
            "top_adverse_reactions": [
                {"reaction_term": rx.Reactions, "report_count": rx.Count} for rx in self.results["top_reactions"]
            ],
            "top_clinical_outcomes": self.results["top_outcomes"].to_dict(orient="records"),
            "top_concomitant_drugs": self.results["top_interacting_drugs"].to_dict(orient="records")
        }
    ...
    def to_csv(self, output_path: Path) -> None:
        ...
        with open(output_path, 'w', newline='', encoding='utf-8') as f:
            ...
            writer.writerow(["Metadata", "Search", "target_drug", self.results["drug_name"]])
            writer.writerow(["Metadata", "Search", "total_reports", self.results["total_reports"]])
            ...
            writer.writerow(["Metrics", "Age", "average_age", f"{self.results['age_demographics']['average_age']:.1f}"])
            for row in self.results["age_demographics"]["cohorts"].itertuples(index=False):
            ...
            for idx, rx in enumerate(self.results["top_reactions"], 1):
                writer.writerow(["Top_Adverse_Reactions", f"Rank_{idx}", rx.Reactions, rx.Count])

            for idx, row in enumerate(self.results["top_outcomes"].itertuples(index=False), 1):
                writer.writerow(["Clinical_Outcomes", f"Rank_{idx}", row.Outcome, f"{row.Percentage:.1f}% (n={row.Count})"])

            for idx, row in enumerate(self.results["top_interacting_drugs"].itertuples(index=False), 1):
